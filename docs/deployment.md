...
docker run --rm \
  ...
  spacl:0.1.0 coordinator --bind 0.0.0.0:8080 --data-dir /var/lib/spacl
```
...
Do not deploy v0.1.0 on a physical robot. Before a production pilot, complete all required work in [Security Model](security.md). Add service authentication, encrypted transport, operator authentication, backup recovery tests, dependency scanning, metrics, rate limits, and an external security review.
